...
async function handleCapturesInit(_req, res, config) {
    ...
    const bucket = process.env.CAPTURE_AUDIO_BUCKET || config.captureAudioBucket || '';
    if (!bucket) {
        writeJson(res, 500, {
...
function encodePathForGcs(objectPath) {
...
function httpsJson(options, body, extraHeaders) {
...
async function signBlobWithIamCredentials(serviceAccountEmail, bytesToSign) {
    const token = await getAccessToken();
    const body = {
...
async function generateV4SignedPutUrl(args) {
    ...
    const signatureBytes = await signBlobWithIamCredentials(serviceAccountEmail, Buffer.from(stringToSign, 'utf8'));
