...
function makeTargetPatterns(target) {
    switch (target) {
        case "dangling":
            return [/\bdangl(?:e|ed|es|ing)\b/i];
        case "boxed-in":
            return [/\bboxed[- ]in\b/i];
        case "sealed":
            return [/\bseal(?:ed|s|ing)?\b/i];
        case "trapped":
            return [/\btrap(?:ped|s|ping)?\b/i];
        case "locked-in":
            return [/\blocked[- ]in\b/i];
        case "narrowed":
            return [/\bnarrow(?:ed|s|ing)?\b/i];
        default:
...
export function scoreClosureDriveByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreClosureDriveFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreClosureDriveNeutral();
    }
}
function scoreClosureDriveFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "JP.closureDrive.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "JP.closureDrive.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "JP.closureDrive.freeRecall.first_hit_social"
                : "JP.closureDrive.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "JP.closureDrive.freeRecall.early_social_bias"
                : "JP.closureDrive.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "JP.closureDrive.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "JP.closureDrive.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = closureDriveConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildClosureDriveScore(score, confidence, cues);
}
function scoreClosureDriveNeutral() {
    return buildClosureDriveScore(0.5, 0.25, []);
}
function closureDriveConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildClosureDriveScore(score, confidence, cues) {
    return {
