...
function makeTargetPatterns(target) {
    switch (target) {
        case "biased":
            return [/\bbias(?:ed|es)?\b/i];
        case "clouded":
            return [/\bcloud(?:ed|ing)?\b/i];
        case "swayed":
            return [/\bsway(?:ed|ing|s)?\b/i];
        case "sentimental":
            return [/\bsentimental(?:ity)?\b/i];
        case "contaminated":
            return [/\bcontaminat(?:e|ed|es|ing|ion)\b/i];
        case "inhuman":
            return [/\binhuman(?:ity)?\b/i];
        case "detached":
            return [/\bdetach(?:ed|es|ing)?\b/i];
        default:
...
export function scoreDecisionSubstrateByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreDecisionSubstrateFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreDecisionSubstrateNeutral();
    }
}
function scoreDecisionSubstrateFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.decisionSubstrate.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.decisionSubstrate.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "TF.decisionSubstrate.freeRecall.first_hit_social"
                : "TF.decisionSubstrate.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "TF.decisionSubstrate.freeRecall.early_social_bias"
                : "TF.decisionSubstrate.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.decisionSubstrate.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.decisionSubstrate.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = decisionSubstrateConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildDecisionSubstrateScore(score, confidence, cues);
}
function scoreDecisionSubstrateNeutral() {
    return buildDecisionSubstrateScore(0.5, 0.25, []);
}
function decisionSubstrateConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildDecisionSubstrateScore(score, confidence, cues) {
    return {
