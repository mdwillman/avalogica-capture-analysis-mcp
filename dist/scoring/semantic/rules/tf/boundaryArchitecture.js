...
function makeTargetPatterns(target) {
    switch (target) {
        case "invasive":
            return [/\binvas(?:ive|iveness)\b/i];
        case "smothering":
            return [/\bsmother(?:ing|ed|s)?\b/i];
        case "intrusive":
            return [/\bintrus(?:ive|iveness)\b/i];
        case "disconnected":
            return [/\bdisconnect(?:ed|s|ing)?\b/i];
        case "guarded":
            return [/\bguard(?:ed|s|ing)?\b/i];
        case "estranged":
            return [/\bestrang(?:ed|es|ing)?\b/i];
        case "withheld":
            return [/\bwithhold(?:s|ing|en)?\b/i, /\bwithheld\b/i];
        default:
...
export function scoreBoundaryArchitectureByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreBoundaryArchitectureFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreBoundaryArchitectureNeutral();
    }
}
function scoreBoundaryArchitectureFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.boundaryArchitecture.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.boundaryArchitecture.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "TF.boundaryArchitecture.freeRecall.first_hit_social"
                : "TF.boundaryArchitecture.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "TF.boundaryArchitecture.freeRecall.early_social_bias"
                : "TF.boundaryArchitecture.freeRecall.early_solitary_bias",
            weight: earlyBias * 0.04,
            text: earlyBias > 0 ? "first three recalls lean F" : "first three recalls lean T",
        });
    }
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.boundaryArchitecture.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.boundaryArchitecture.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = boundaryArchitectureConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildBoundaryArchitectureScore(score, confidence, cues);
}
function scoreBoundaryArchitectureNeutral() {
    return buildBoundaryArchitectureScore(0.5, 0.25, []);
}
function boundaryArchitectureConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildBoundaryArchitectureScore(score, confidence, cues) {
    return {
