...
function makeTargetPatterns(target) {
    switch (target) {
        case "incompetent":
            return [/\bincompet(?:ent|ence)\b/i];
        case "botched":
            return [/\bbotch(?:ed|es|ing)?\b/i];
        case "incoherent":
            return [/\bincoher(?:ent|ence)\b/i];
        default:
...
export function scoreEvaluationReflexByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreEvaluationReflexFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreEvaluationReflexNeutral();
    }
}
function scoreEvaluationReflexFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.evaluationReflex.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.evaluationReflex.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "TF.evaluationReflex.freeRecall.first_hit_social"
                : "TF.evaluationReflex.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "TF.evaluationReflex.freeRecall.early_social_bias"
                : "TF.evaluationReflex.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.evaluationReflex.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "TF.evaluationReflex.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = evaluationReflexConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildEvaluationReflexScore(score, confidence, cues);
}
function scoreEvaluationReflexNeutral() {
    return buildEvaluationReflexScore(0.5, 0.25, []);
}
function evaluationReflexConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildEvaluationReflexScore(score, confidence, cues) {
    return {
