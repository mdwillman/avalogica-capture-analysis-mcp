...
function makeTargetPatterns(target) {
    switch (target) {
        ...
        case "hemmed-in":
            return [/\bhemmed[ -]?in\b/i];
        case "far-off":
            return [/\bfar[ -]?off\b/i];
        case "deferred":
            return [/\bdefer(?:red|ring|s|r)?\b/i];
        case "hypothetical":
            return [/\bhypothetical(?:ly)?\b/i];
        default:
...
export function scoreTemporalHabitatByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreTemporalHabitatFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreTemporalHabitatNeutral();
    }
}
function scoreTemporalHabitatFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.temporalHabitat.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.temporalHabitat.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "NS.temporalHabitat.freeRecall.first_hit_social"
                : "NS.temporalHabitat.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "NS.temporalHabitat.freeRecall.early_social_bias"
                : "NS.temporalHabitat.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.temporalHabitat.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.temporalHabitat.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = temporalHabitatConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildTemporalHabitatScore(score, confidence, cues);
}
function scoreTemporalHabitatNeutral() {
    return buildTemporalHabitatScore(0.5, 0.25, []);
}
function temporalHabitatConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildTemporalHabitatScore(score, confidence, cues) {
    return {
