...
function makeTargetPatterns(target) {
    switch (target) {
        case "hand-wavy":
            return [/\bhand[ -]?wavy\b/i];
        case "bogged-down":
            return [/\bbogged[ -]?down\b/i];
        case "inflated":
            return [/\binflat(?:ed|ing|e|es)\b/i];
        case "overgeneralized":
            return [/\bovergeneraliz(?:ed|ing|e|es)\b/i];
        case "ungrounded":
            return [/\bunground(?:ed|ing)?\b/i];
        default:
...
export function scoreInformationDietByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreInformationDietFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreInformationDietNeutral();
    }
}
function scoreInformationDietFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.informationDiet.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.informationDiet.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "NS.informationDiet.freeRecall.first_hit_social"
                : "NS.informationDiet.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "NS.informationDiet.freeRecall.early_social_bias"
                : "NS.informationDiet.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.informationDiet.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.informationDiet.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = informationDietConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildInformationDietScore(score, confidence, cues);
}
function scoreInformationDietNeutral() {
    return buildInformationDietScore(0.5, 0.25, []);
}
function informationDietConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildInformationDietScore(score, confidence, cues) {
    return {
