...
export function scoreMeaningThresholdByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
...
function scoreMeaningThresholdFreeRecall(transcriptRaw) {
...
function scoreMeaningThresholdNeutral() {
    return buildMeaningThresholdScore(0.5, 0.25, []);
}
function meaningThresholdConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildMeaningThresholdScore(score, confidence, cues) {
    return {
