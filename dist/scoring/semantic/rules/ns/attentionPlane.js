...
function makeTargetPatterns(target) {
    switch (target) {
        case "speculative":
            return [/\bspeculat(?:ive|e|es|ed|ing|ion)\b/i];
        case "ungrounded":
...
export function scoreAttentionPlaneByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreAttentionPlaneFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreAttentionPlaneNeutral();
    }
}
function scoreAttentionPlaneFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.attentionPlane.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.attentionPlane.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "NS.attentionPlane.freeRecall.first_hit_social"
                : "NS.attentionPlane.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "NS.attentionPlane.freeRecall.early_social_bias"
                : "NS.attentionPlane.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.attentionPlane.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.attentionPlane.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = attentionPlaneConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildAttentionPlaneScore(score, confidence, cues);
}
function scoreAttentionPlaneNeutral() {
    return buildAttentionPlaneScore(0.5, 0.25, []);
}
function attentionPlaneConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildAttentionPlaneScore(score, confidence, cues) {
    return {
