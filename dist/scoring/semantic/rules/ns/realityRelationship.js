...
function makeTargetPatterns(target) {
    switch (target) {
        case "far-fetched":
            return [/\bfar[ -]?fetched\b/i];
        case "untethered":
            return [/\buntether(?:ed|ing)?\b/i];
        case "untested":
            return [/\buntest(?:ed|ing)?\b/i];
        default:
...
export function scoreRealityRelationshipByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreRealityRelationshipFreeRecall(transcript);
        case "openInterpretation":
        case "microDecision":
        case "derailmentRecognition":
        case "projectionContinuation":
        default:
            return scoreRealityRelationshipNeutral();
    }
}
function scoreRealityRelationshipFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.realityRelationship.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.realityRelationship.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "NS.realityRelationship.freeRecall.first_hit_social"
                : "NS.realityRelationship.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "NS.realityRelationship.freeRecall.early_social_bias"
                : "NS.realityRelationship.freeRecall.early_solitary_bias",
            weight: earlyBias * 0.04,
            text: earlyBias > 0 ? "first three recalls lean S" : "first three recalls lean N",
        });
    }
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.realityRelationship.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "NS.realityRelationship.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = realityRelationshipConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildRealityRelationshipScore(score, confidence, cues);
}
function scoreRealityRelationshipNeutral() {
    return buildRealityRelationshipScore(0.5, 0.25, []);
}
function realityRelationshipConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
function buildRealityRelationshipScore(score, confidence, cues) {
    return {
