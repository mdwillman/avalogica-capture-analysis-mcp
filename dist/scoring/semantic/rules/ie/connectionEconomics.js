...
function connectionConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
export function scoreConnectionEconomicsByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
...
function scoreConnectionEconomicsFreeRecall(transcriptRaw) {
...
function scoreConnectionEconomicsNeutral() {
    return buildConnectionEconomicsScore(0.5, 0.25, []);
}
function buildConnectionEconomicsScore(score, confidence, cues) {
    return {
