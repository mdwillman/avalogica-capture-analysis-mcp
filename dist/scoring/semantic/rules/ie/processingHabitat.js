...
function makeTargetPatterns(target) {
    switch (target) {
        case "scattered":
            return [/\bscatter(?:ed|ing|s)?\b/i];
        case "fragmented":
            return [/\bfragment(?:ed|ing|s)?\b/i];
        case "cluttered":
            return [/\bclutter(?:ed|ing|s)?\b/i];
        case "interrupted":
            return [/\binterrupt(?:ed|ing|s)?\b/i];
        case "invaded":
            return [/\binvad(?:ed|ing|es|e|ers|ion)\b/i];
        default:
...
function processingHabitatConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
export function scoreProcessingHabitatByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
            return scoreProcessingHabitatFreeRecall(transcript);
        default:
            return scoreProcessingHabitatNeutral();
    }
}
function scoreProcessingHabitatFreeRecall(transcriptRaw) {
    ...
    if (socialHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "IE.processingHabitat.freeRecall.social_target_hits",
    ...
    if (solitaryHits > 0) {
        cues.push({
            kind: "semantic",
            featureId: "IE.processingHabitat.freeRecall.solitary_target_hits",
    ...
    if (firstHit) {
        cues.push({
            ...
            featureId: firstHit.category === "social"
                ? "IE.processingHabitat.freeRecall.first_hit_social"
                : "IE.processingHabitat.freeRecall.first_hit_solitary",
    ...
    if (earlyBias !== 0) {
        cues.push({
            ...
            featureId: earlyBias > 0
                ? "IE.processingHabitat.freeRecall.early_social_bias"
                : "IE.processingHabitat.freeRecall.early_solitary_bias",
    ...
    if (sameCategoryPairs > 0) {
        cues.push({
            kind: "semantic",
            featureId: "IE.processingHabitat.freeRecall.category_clustering",
    ...
    if (totalHits === 0) {
        cues.push({
            kind: "semantic",
            featureId: "IE.processingHabitat.freeRecall.no_target_recall",
            weight: 0,
            text: "no card target terms were recalled",
        });
    }
    const totalSignals = totalHits + (firstHit ? 1 : 0) + Math.abs(earlyBias) + sameCategoryPairs;
    const confidence = processingHabitatConfidence(totalSignals, analysis.confidenceMultiplier, 0.26, 0.05);
    return buildProcessingHabitatScore(score, confidence, cues);
}
function scoreProcessingHabitatNeutral() {
    return buildProcessingHabitatScore(0.5, 0.25, []);
}
function buildProcessingHabitatScore(score, confidence, cues) {
    return {
