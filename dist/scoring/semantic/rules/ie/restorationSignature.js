...
function restorationConfidence(totalSignals, multiplier, base = 0.26, step = 0.05) {
    return clamp01((base + Math.min(0.45, totalSignals * step)) * multiplier);
}
export function scoreRestorationSignatureByPromptType(transcript, promptType) {
    switch (promptType) {
        case "freeRecall":
...
function scoreRestorationSignatureFreeRecall(transcriptRaw) {
...
function scoreRestorationSignatureNeutral() {
    return buildRestorationSignatureScore(0.5, 0.25, []);
}
function buildRestorationSignatureScore(score, confidence, cues) {
    return {
