...
function isConditionalNegation(tokens, negTokenIndex, effectiveFrom) {
    ...
    for (let i = negTokenIndex - 1; i >= effectiveFrom && i >= negTokenIndex - lookback; i--) {
        ...
        if (!w)
            continue;
        if (CONDITIONAL_CLAUSE_BREAK_TOKENS.has(w))
            break;
        if (CONDITIONAL_INTROS.has(w))
            return true;
    }
    return false;
}
...
export function findNegationBeforeIndex(text, tokens, phraseStartTokenIndex, opts) {
    ...
    for (let i = phraseStartTokenIndex - 1; i >= effectiveFrom; i--) {
        ...
        if (isConditionalNegation(tokens, i, effectiveFrom))
            continue;
        return tok;
    }
    return undefined;
