import { ANSWER_DIRECTNESSES, BEHAVIOR_ADMISSIONS, COMPULSION_STANCES, EVIDENCE_STRENGTHS, JUSTIFICATION_MODES, METAPHOR_STANCES, NORMATIVE_POSTURES, PARSE_STATUSES, RESPONSE_STYLES, SCORABLE_CLARITIES, SEMANTIC_PARSE_SCHEMA_VERSION, SELF_EVALUATIONS, STATE_RELATIONS, SURFACE_POLARITIES, IE_PROMPT_AXIS_INTERPRETATIONS, JP_PROMPT_AXIS_INTERPRETATIONS, NS_PROMPT_AXIS_INTERPRETATIONS, TF_PROMPT_AXIS_INTERPRETATIONS, TRANSCRIPT_QUALITIES, } from "./types.js";
const DEFAULT_PARSER_MODEL = process.env.SEMANTIC_PARSER_MODEL ?? "gemini-2.5-flash-lite";
const DEFAULT_SEMANTIC_PARSER_VERSION = process.env.SEMANTIC_PARSER_VERSION ?? "derailment_semantic_parser_v1";
const SUPPORTED_PARSER_DIMENSIONS = new Set(["TF", "IE", "JP", "NS"]);
...
let parserModelHandlePromise = null;
export async function runSemanticParser(request) {
    if (!SUPPORTED_PARSER_DIMENSIONS.has(request.prompt.dimensionId))
        return undefined;
...
export function validateSemanticParseResult(result, prompt) {
    if (!isPlainObject(result))
        return undefined;
    if (result.schemaVersion !== SEMANTIC_PARSE_SCHEMA_VERSION)
        return undefined;
    if (result.promptId && result.promptId !== prompt.id)
        return undefined;
    if (result.dimension !== prompt.dimensionId)
        return undefined;
    if (!isOneOf(result.parseStatus, PARSE_STATUSES))
        return undefined;
    if (!isOneOf(result.transcriptQuality, TRANSCRIPT_QUALITIES))
        return undefined;
    if (typeof result.responseHasSubstantiveContent !== "boolean")
        return undefined;
    if (!validateFields(result.fields))
        return undefined;
    if (!validateScoringHints(result.scoringHints))
        return undefined;
    if (!validateParserMeta(result.parserMeta))
        return undefined;
    return result;
}
function validateFields(fields) {
    return (isOneOf(fields.surfacePolarity, SURFACE_POLARITIES) &&
...
function validateScoringHints(hints) {
    return ((hints.tfPromptAxisInterpretation === undefined ||
...
function validateParserMeta(meta) {
    if (!meta)
        return false;
    if (typeof meta.parserModel !== "string" || typeof meta.parserVersion !== "string")
        return false;
    if (typeof meta.parserConfidence !== "number" || Number.isNaN(meta.parserConfidence))
        return false;
    if (meta.notesForDebug !== undefined && typeof meta.notesForDebug !== "string")
        return false;
    return true;
}
...
function buildParserPayload(request, transcript) {
    const payload = {
...
function normalizeSemanticParseResult(result) {
    let normalized = result;
    if (result.fields.metaphorStance === "rejects_metaphor_accepts_behavior" &&
    ...
    if (normalized.promptId === "VK.TF.4A.v1" &&
    ...
    if (normalized.dimension === "TF") {
        let normalizedInterpretation = normalizeTfPromptAxisInterpretation(normalized.promptId, normalized.scoringHints.tfPromptAxisInterpretation);
        if (normalized.promptId === "VK.TF.4A.v1" &&
            normalized.fields.metaphorStance === "accepts_metaphor" &&
            (normalized.fields.behaviorAdmission === "admits" ||
                (normalized.fields.behaviorAdmission === "unclear" &&
        ...
        const normalizedIE = normalizeIePromptAxisInterpretation(normalized.promptId, normalized.scoringHints.iePromptAxisInterpretation);
        ...
        const normalizedJP = normalizeJpPromptAxisInterpretation(normalized.promptId, normalized.scoringHints.jpPromptAxisInterpretation);
        ...
        const normalizedNS = normalizeNsPromptAxisInterpretation(normalized.promptId, normalized.scoringHints.nsPromptAxisInterpretation);
...
function normalizeTfPromptAxisInterpretation(promptId, interpretation) {
    if (!interpretation || interpretation === "unclear") {
        return interpretation;
    }
    const allowedByPrompt = {
        "VK.TF.4A.v1": new Set([
...
function normalizeIePromptAxisInterpretation(promptId, interpretation) {
    if (!interpretation || interpretation === "unclear") {
        return interpretation;
    }
    const allowedByPrompt = {
...
function normalizeJpPromptAxisInterpretation(promptId, interpretation) {
    if (!interpretation || interpretation === "unclear") {
        return interpretation;
    }
    const allowedByPrompt = {
...
function normalizeNsPromptAxisInterpretation(promptId, interpretation) {
    if (!interpretation || interpretation === "unclear") {
        return interpretation;
    }
    const allowedByPrompt = {
...
function applyParserMetadata(result, modelName) {
