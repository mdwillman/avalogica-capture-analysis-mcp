...
export type AspirationKind = "aspiration" | "obligation" | "actual";

export type AspirationHit = TextSpan & {
  phrase: string;
  kind: AspirationKind;
  strength: "weak" | "medium" | "strong";
};

export type AspirationOptions = {
  /** If true, include informal variants ("wanna", "I'd wanna"). Default true. */
  includeInformal?: boolean;
  /** If true, enable conservative heuristics to prune false positives. Default true. */
  enableHeuristics?: boolean;
};

const DEFAULT_OPTS: Required<AspirationOptions> = {
...
export type AspirationSummary = {
  hits: AspirationHit[];
  counts: {
    aspiration: { weak: number; medium: number; strong: number };
    obligation: { weak: number; medium: number; strong: number };
    actual: { weak: number; medium: number; strong: number };
  };
  /**
   * A coarse scalar in [-1,+1] representing "ideal-self vs actual" balance.
   * - positive => more actual-reporting language
   * - negative => more aspiration/obligation language
   */
  actualityIndex: number;
};

export function summarizeAspiration(text: string, opts?: AspirationOptions): AspirationSummary {
  ...
  const pos =
    counts.actual.strong * 1.0 +
    counts.actual.medium * 0.6 +
    counts.actual.weak * 0.35;

  const neg =
    (counts.aspiration.strong + counts.obligation.strong) * 1.0 +
 ...
 */
export function aspirationConfidenceMultiplier(actualityIndex: number): number {
 ...
 */
export function hasIdealVsActualTension(summary: AspirationSummary): boolean {
...
function pruneFalsePositives(text: string, hits: AspirationHit[]): AspirationHit[] {
  const lower = normalize(text);
  ...
  return hits.filter((h) => {
    ...
    if (p === "in general" && h.kind === "actual") {
      ...
      if (/speaking\s+in\s+general/.test(ctx)) return false;
    }
