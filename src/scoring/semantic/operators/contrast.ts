...
export type ContrastSplit = {
  /** Text before the contrast marker (trimmed). */
  pre: string;
  /** Text after the contrast marker (trimmed). */
  post: string;
  /** Span for pre in original text (rough; excludes leading/trailing whitespace). */
  preSpan: TextSpan;
  /** Span for post in original text (rough; excludes leading/trailing whitespace). */
  postSpan: TextSpan;
  /** The contrast marker chosen (strongest/latest). */
  marker: ContrastMarker;
};

export type ContrastOptions = {
  /** If true, tries to ignore "not only ... but also ..." (not contrastive). Default true. */
  ignoreNotOnlyButAlso?: boolean;
  /** If true, include conversational pivots like "actually", "to be honest". Default true. */
  includePivots?: boolean;
};

const DEFAULT_OPTS: Required<ContrastOptions> = {
 ...
 */
export function findContrastMarkers(text: string, opts?: ContrastOptions): ContrastMarker[] {
  const o = { ...DEFAULT_OPTS, ...(opts ?? {}) };
  const lower = normalize(text);

  const markers = [...CORE_MARKERS, ...(o.includePivots ? PIVOT_MARKERS : [])];
  const out: ContrastMarker[] = [];
  ...
  if (o.ignoreNotOnlyButAlso) {
    return out.filter((mk) => {
      if (mk.marker.toLowerCase() !== "but") return true;
 ...
 */
export function chooseMainContrastMarker(text: string, opts?: ContrastOptions): ContrastMarker | undefined {
 ...
 */
export function splitOnMainContrast(text: string, opts?: ContrastOptions): ContrastSplit | undefined {
  ...
  let preStart = 0;
  {
    ...
    if (lastBreak >= 0) preStart = lastBreak + 1;
  }
...
export type ContrastWeightedText = {
  /** Preferred/stronger segment (typically after contrast). */
  primary: string;
  primarySpan: TextSpan;
  /** Secondary/weaker segment (typically before contrast). */
  secondary: string;
  secondarySpan: TextSpan;
  /** Marker info if a contrast was detected. */
  marker?: ContrastMarker;
  /** True if a contrast was detected and weighting applied. */
  hasContrast: boolean;
};
 ...
 * - secondary = ""
 */
export function contrastWeightedText(text: string, opts?: ContrastOptions): ContrastWeightedText {
 ...
 */
export function filterSpansWithin<T extends TextSpan>(spans: T[], within: TextSpan): T[] {
