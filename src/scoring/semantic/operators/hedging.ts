...
export type HedgeStrength = "weak" | "medium" | "strong";

export type HedgeHit = TextSpan & {
  phrase: string;
  strength: HedgeStrength;
};

export type HedgingOptions = {
  /** If true, include informal fillers ("like", "um", "uh"). Default false (conservative). */
  includeFillers?: boolean;
  /** If true, include ultra-common softenors ("pretty", "somewhat"). Default true. */
  includeSoftenors?: boolean;
  /** If true, attempt to prune some false positives. Default true. */
  enableHeuristics?: boolean;
};

const DEFAULT_OPTS: Required<HedgingOptions> = {
...
function findPhraseHits(text: string, phrases: string[], strength: HedgeStrength): HedgeHit[] {
...
export type HedgingSummary = {
  hits: HedgeHit[];
  counts: { weak: number; medium: number; strong: number };
  /**
   * A coarse scalar in [0,1] representing total hedge intensity.
   * 0 = no hedging detected; 1 = heavy hedging.
   */
  hedgeIndex01: number;
};

export function summarizeHedging(text: string, opts?: HedgingOptions): HedgingSummary {
  ...
  for (const h of cleaned) counts[h.strength] += 1;
 ...
 */
export function hedgingConfidenceMultiplier(hedgeIndex01: number): number {
...
function pruneFalsePositives(text: string, hits: HedgeHit[]): HedgeHit[] {
  const lower = normalize(text);
  ...
  return hits.filter((h) => {
    ...
    if (p === "like") {
      ...
      if (/like\s+to/.test(ctx)) return false;
    }
