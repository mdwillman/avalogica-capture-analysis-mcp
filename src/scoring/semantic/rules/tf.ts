 ...
 */
export function scoreTFEvaluationReflex(transcriptRaw: string): {
  score01: number;
  confidence01: number;
  cues: any[];
} {
  ...
  const anyStrongAnchor =
    strongFEffective + strongTEffective + strongFNegated + strongTNegated > 0;
  if (anyStrongAnchor) delta *= 1.25;
  ...
  // Strong anchors
  confidence01 += Math.min(
    0.30,
    (strongFEffective + strongTEffective + strongFNegated + strongTNegated) * 0.18
  );
 ...
 * Reminder: TF mapping is low=T, high=F.
 */
export function scoreTFTruthOrientation(transcriptRaw: string): {
  score01: number;
  confidence01: number;
  cues: any[];
} {
  ...
  confidence01 += Math.min(
    0.32,
    (effectiveRules + effectiveEquity + negatedRules + negatedEquity) * 0.08
  );
 ...
 * Reminder: TF mapping is low=T, high=F.
 */
export function scoreTFDecisionSubstrate(transcriptRaw: string): {
  score01: number;
  confidence01: number;
  cues: any[];
} {
  ...
  const anyStrongAnchor =
    strongTEffective + strongFEffective + strongTNegated + strongFNegated > 0;
  if (anyStrongAnchor) delta *= 1.25;
  ...
  confidence01 += Math.min(
    0.30,
    (strongTEffective + strongFEffective + strongTNegated + strongFNegated) * 0.18
  );
 ...
 * Reminder: TF mapping is low=T, high=F.
 */
export function scoreTFConflictMetabolism(transcriptRaw: string): {
  score01: number;
  confidence01: number;
  cues: any[];
} {
 ...
 * Reminder: TF mapping is low=T, high=F.
 */
export function scoreTFBoundaryArchitecture(transcriptRaw: string): {
  score01: number;
  confidence01: number;
  cues: any[];
} {
  ...
  confidence01 += Math.min(
    0.32,
    (effectiveFlaws + effectiveStrengths + negatedFlaws + negatedStrengths) * 0.08
  );
