...
export function scoreRealityRelationshipByPromptType(
  transcript: string,
...
function scoreRealityRelationshipFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreRealityRelationshipNeutral(): SubAxisScore {
  return buildRealityRelationshipScore(0.5, 0.25, []);
}

function realityRelationshipConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildRealityRelationshipScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
