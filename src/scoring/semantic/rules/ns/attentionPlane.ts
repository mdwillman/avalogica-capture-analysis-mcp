...
export function scoreAttentionPlaneByPromptType(
  transcript: string,
...
function scoreAttentionPlaneFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreAttentionPlaneNeutral(): SubAxisScore {
  return buildAttentionPlaneScore(0.5, 0.25, []);
}

function attentionPlaneConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildAttentionPlaneScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
