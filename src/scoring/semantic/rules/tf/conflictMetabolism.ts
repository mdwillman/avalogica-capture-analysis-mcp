...
export function scoreConflictMetabolismByPromptType(
  transcript: string,
...
function scoreConflictMetabolismFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreConflictMetabolismNeutral(): SubAxisScore {
  return buildConflictMetabolismScore(0.5, 0.25, []);
}

function conflictMetabolismConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildConflictMetabolismScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
