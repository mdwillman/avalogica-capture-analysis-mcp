...
export function scoreTruthOrientationByPromptType(
  transcript: string,
...
function scoreTruthOrientationFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreTruthOrientationNeutral(): SubAxisScore {
  return buildTruthOrientationScore(0.5, 0.25, []);
}

function truthOrientationConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildTruthOrientationScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
