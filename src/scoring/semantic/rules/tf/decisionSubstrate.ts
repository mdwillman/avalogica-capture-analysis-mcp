...
export function scoreDecisionSubstrateByPromptType(
  transcript: string,
...
function scoreDecisionSubstrateFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreDecisionSubstrateNeutral(): SubAxisScore {
  return buildDecisionSubstrateScore(0.5, 0.25, []);
}

function decisionSubstrateConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildDecisionSubstrateScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
