...
export function scoreBoundaryArchitectureByPromptType(
  transcript: string,
...
function scoreBoundaryArchitectureFreeRecall(transcriptRaw: string): SubAxisScore {
  ...
  const confidence = boundaryArchitectureConfidence(
    totalSignals,
    analysis.confidenceMultiplier,
    0.26,
    0.05,
  );
...
function scoreBoundaryArchitectureNeutral(): SubAxisScore {
  return buildBoundaryArchitectureScore(0.5, 0.25, []);
}

function boundaryArchitectureConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildBoundaryArchitectureScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
