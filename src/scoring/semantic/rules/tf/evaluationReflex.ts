...
export function scoreEvaluationReflexByPromptType(
  transcript: string,
...
function scoreEvaluationReflexFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreEvaluationReflexNeutral(): SubAxisScore {
  return buildEvaluationReflexScore(0.5, 0.25, []);
}

function evaluationReflexConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildEvaluationReflexScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
