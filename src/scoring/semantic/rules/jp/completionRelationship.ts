...
export function scoreCompletionRelationshipByPromptType(
  transcript: string,
...
function scoreCompletionRelationshipFreeRecall(transcriptRaw: string): SubAxisScore {
  ...
  const confidence = completionRelationshipConfidence(
    totalSignals,
...
function scoreCompletionRelationshipNeutral(): SubAxisScore {
  return buildCompletionRelationshipScore(0.5, 0.25, []);
}

function completionRelationshipConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildCompletionRelationshipScore(
  score: number,
  confidence: number,
  cues: any[],
): SubAxisScore {
