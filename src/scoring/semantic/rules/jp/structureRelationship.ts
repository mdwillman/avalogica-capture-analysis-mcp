...
export function scoreStructureRelationshipByPromptType(
  transcript: string,
...
function scoreStructureRelationshipFreeRecall(transcriptRaw: string): SubAxisScore {
  ...
  const confidence = structureRelationshipConfidence(
    totalSignals,
...
function scoreStructureRelationshipNeutral(): SubAxisScore {
  return buildStructureRelationshipScore(0.5, 0.25, []);
}

function structureRelationshipConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildStructureRelationshipScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
