...
export function scoreTemporalOrientationToPlansByPromptType(
  transcript: string,
...
function scoreTemporalOrientationToPlansFreeRecall(transcriptRaw: string): SubAxisScore {
  ...
  const confidence = temporalOrientationToPlansConfidence(
    totalSignals,
...
function scoreTemporalOrientationToPlansNeutral(): SubAxisScore {
  return buildTemporalOrientationToPlansScore(0.5, 0.25, []);
}

function temporalOrientationToPlansConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildTemporalOrientationToPlansScore(
  score: number,
  confidence: number,
  cues: any[],
): SubAxisScore {
