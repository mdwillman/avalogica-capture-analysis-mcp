...
export function scoreClosureDriveByPromptType(
  transcript: string,
...
function scoreClosureDriveFreeRecall(transcriptRaw: string): SubAxisScore {
...
function scoreClosureDriveNeutral(): SubAxisScore {
  return buildClosureDriveScore(0.5, 0.25, []);
}

function closureDriveConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildClosureDriveScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
