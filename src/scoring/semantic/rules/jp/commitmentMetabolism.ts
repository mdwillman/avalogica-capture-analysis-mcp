...
export function scoreCommitmentMetabolismByPromptType(
  transcript: string,
...
function scoreCommitmentMetabolismFreeRecall(transcriptRaw: string): SubAxisScore {
  ...
  const confidence = commitmentMetabolismConfidence(
    totalSignals,
...
function scoreCommitmentMetabolismNeutral(): SubAxisScore {
  return buildCommitmentMetabolismScore(0.5, 0.25, []);
}

function commitmentMetabolismConfidence(
  totalSignals: number,
  multiplier: number,
  base = 0.26,
  step = 0.05,
...
function buildCommitmentMetabolismScore(score: number, confidence: number, cues: any[]): SubAxisScore {
  return {
